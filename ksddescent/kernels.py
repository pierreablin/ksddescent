...
def linear_stein_kernel(x, y, score_x, score_y, return_kernel=False):
    ...
    n, d = x.shape
    kernel = x @ y.t()
    stein_kernel = score_x @ score_y.t() * kernel  + score_x @ x.t() + score_y @ y.t() + d
    if return_kernel:
...
def imq_kernel(x, y, score_x, score_y, g=1, beta=.5, return_kernel=False):
    ...
    d2kxy = 2 * (beta * g * (res) ** (-beta - 1) * p - 2 * beta * (beta + 1) * g ** 2 * dists * res ** (-beta - 2))
